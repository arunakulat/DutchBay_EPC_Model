...
class TypeChecker(NodeVisitor[None], TypeCheckerSharedApi):
    ...
    def _visit_overloaded_func_def(self, defn: OverloadedFuncDef) -> None:
        ...
        if defn.is_property:
            ...
            if defn.items[0].var.is_settable_property:
                ...
                if (
                    not isinstance(setter_type, CallableType)
                    or len(setter_type.arg_types) != 2
                ):
                ...
                if isinstance(defn.type, Overloaded):
                    ...
                    getter_type = self.extract_callable_type(
                        defn.items[0].var.type, defn
                    )
            ...
            if defn.is_property:
                ...
                if (settable and i > 1) or (not settable and i > 0):
                    self.check_func_item(
                        fdef.func, name=fdef.func.name, allow_empty=True
                    )
            else:
    ...
    def check_setter_type_override(
        self, defn: OverloadedFuncDef, base: TypeInfo
    ) -> None:
        ...
        original_type, _ = self.node_type_from_base(
            defn.name, base, defn, setter_type=True
        )
    ...
    def check_method_override_for_base_with_name(
        ...
        if isinstance(original_type, AnyType) or isinstance(typ, AnyType):
            ...
            original_node
            and (
                not self.is_writable_attribute(original_node) or always_allow_covariant
            )
    ...
    def visit_class_def(self, defn: ClassDef) -> None:
        ...
        if not can_have_shared_disjoint_base(typ.bases):
            self.fail(
                message_registry.INCOMPATIBLE_DISJOINT_BASES.format(typ.name), defn
            )
        with (
    ...
    def check_typevar_defaults(self, tvars: Sequence[TypeVarLikeType]) -> None:
        for tv in tvars:
            ...
            if tv.values and not any(
                is_same_type(tv.default, value) for value in tv.values
            ):
    ...
    def check_compatibility(
        ...
        if isinstance(p_first_type, FunctionLike) and isinstance(
            p_second_type, FunctionLike
        ):
    ...
    def get_variable_type_context(
        self, inferred: Var, rvalue: Expression
    ) -> Type | None:
        type_contexts = []
        if inferred.info:
    ...
    def check_compatibility_all_supers(
        self, lvalue: RefExpr, rvalue: Expression
    ) -> None:
        ...
        if (
            ...
            if lvalue_node.is_inferred and not lvalue_node.explicit_self_type:
                ...
                ctx = (
                    lvalue_node.type
                    if not isinstance(lvalue_node.type, PartialType)
                    else None
                )
                rvalue_type = self.expr_checker.accept(rvalue, ctx)
                actual_lvalue_type = lvalue_node.type
                lvalue_node.type = rvalue_type
            lvalue_type, _ = self.node_type_from_base(
                lvalue_node.name, lvalue_node.info, lvalue
            )
            if lvalue_node.is_inferred and not lvalue_node.explicit_self_type:
            ...
            for base in lvalue_node.info.mro[1:]:
                ...
                base_type, base_node = self.node_type_from_base(
                    lvalue_node.name, base, lvalue
                )
    ...
    def node_type_from_base(
        ...
        *,
        ...
        with self.msg.filter_errors(filter_deprecated=True):
            if is_class:
                fallback = instance.type.metaclass_type or mx.named_type(
                    "builtins.type"
                )
        ...
        with self.binder.accumulate_type_assignments() as assignments:
            for item in rvalue_type.items:
                ...
                for t, lv in zip(transposed, self.flatten_lvalues(lvalues)):
                    ...
                    t.append(
                        self._type_maps[-1].pop(lv, AnyType(TypeOfAny.special_form))
                    )
    ...
    def infer_variable_type(
        ...
        if isinstance(init_type, DeletedType):
            ...
            if name.name != "_" and not self.infer_partial_type(
                name, lvalue, init_type
            ):
        ...
        *,
        ...
        if self.is_stub and isinstance(rvalue, EllipsisExpr):
            ...
            if (
                ...
                if (
                    not is_valid_inferred_type(rvalue_type, self.options)
                ...
                if not is_same_type(inferred.type, new_inferred):
                    ...
                    if not self.refers_to_different_scope(lvalue):
                        lvalue_type = make_simplified_union(
                            [inferred.type, new_inferred]
                        )
                        if not is_same_type(
                            lvalue_type, inferred.type
    ...
    def refers_to_different_scope(self, name: NameExpr) -> bool:
        if name.kind == LDEF:
            ...
            name.kind == GDEF and name.fullname.rpartition(".")[0] != self.tree.fullname
        ):
    ...
    def check_member_assignment(
        ...
        if (
            ...
            rvalue_type, _ = self.check_simple_assignment(
                set_lvalue_type, rvalue, context
            )
        ...
        rvalue_type = (
            rvalue_type if is_subtype(rvalue_type, get_lvalue_type) else get_lvalue_type
        )
        return (
            rvalue_type,
            set_lvalue_type,
            is_subtype(get_lvalue_type, set_lvalue_type),
        )
            ...
            if isinstance(var.type, PartialType):
                ...
                if (
                    ...
                    if (
                        ...
                        new_type = self.named_generic_type(
        ...
        with self.msg.filter_errors(
            ...
            with self.local_type_map as type_map:
                alt_typ = get_proper_type(
                    self.expr_checker.accept(
                        expr, None, allow_none_return=allow_none_func_call
                    )
                )
    ...
    def check_return_stmt(self, s: ReturnStmt) -> None:
        ...
        if defn is not None:
            ...
            if s.expr:
                ...
                # Return with a value.
                if isinstance(
                    s.expr, (CallExpr, ListExpr, TupleExpr, DictExpr, SetExpr, OpExpr)
                ):
                    ...
                    typ = self.infer_context_dependent(
                        s.expr, return_type, allow_none_func_call
                    )
                else:
                    typ = get_proper_type(
                        self.expr_checker.accept(
    ...
    def visit_operator_assignment_stmt(self, s: OperatorAssignmentStmt) -> None:
        ...
        if inplace:
            ...
            rvalue_type, _ = self.expr_checker.check_op(
    ...
    def check_untyped_after_decorator(self, typ: Type, func: FuncDef) -> None:
        if (
            not self.options.disallow_any_decorated
            or self.is_stub
            or self.current_node_deferred
        ):
            return
    ...
    def _make_named_statement_for_match(
        self, s: MatchStmt, subject: Expression
    ) -> Expression:
    ...
    def intersect_instances(
        ...
        def _make_fake_typeinfo_and_full_name(
            ...
            names = [
                format_type_bare(x, options=options, verbosity=2) for x in base_classes_
            ]
    ...
    def check_subtype(
        ...
        *,
        ...
        if (
            ...
            and isinstance(
                subtype, (CallableType, Instance, TupleType, TypedDictType, TypeType)
            )
        ):
            self.msg.report_protocol_problems(
                subtype, supertype, context, parent_error=error
            )
...
def is_valid_inferred_type(
    ...
    if isinstance(proper_type, NoneType):
        ...
        return is_lvalue_final or (
            not is_lvalue_member and options.allow_redefinition_new
        )
