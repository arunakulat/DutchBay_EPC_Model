...
def verify_typealias(
    ...
    if isinstance(stub_target, mypy.types.Instance):
        ...
        yield from verify_typeinfo(
            stub_origin, runtime_origin, object_path, is_alias_target=True
        )
        return
...
def safe_inspect_signature(runtime: Any) -> inspect.Signature | None:
    if (
        ...
        try:
            ...
            return s.replace(
                parameters=[
                    inspect.Parameter("self", parameter_kind),
                    *s.parameters.values(),
                ]
            )
        except Exception:
    ...
    if (
        ...
        if not has_init:
            try:
                ...
                return s.replace(
                    parameters=[
                        inspect.Parameter("cls", parameter_kind),
                        *s.parameters.values(),
                    ]
                )
            except Exception:
...
def safe_print(text: str) -> None:
    ...
    if out_encoding is not None:
        ...
        text = text.encode(out_encoding, errors="replace").decode(
