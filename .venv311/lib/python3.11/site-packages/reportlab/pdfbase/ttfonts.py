...
class TTFont:
    ...
    def __init__(
        ...
        self.shapable = shapable and not any(
            (fnmatch(name, _) for _ in unShapedFontGlob)
        )
    ...
    def hbFace(self):
        ...
        if not face:
            if uharfbuzz is None:
                raise ValueError(
                    "Cannot import uharfbuzz so shaping is not allowed\nplease pip install uharfbuzz"
                )
...
def makeShapedFragWord(w, K=[], V=[]):
    ...
    v = (
        ShapedFragWord
        if klass in (list, ShapedFragWord)
        else type("ShapedFragWord", (klass, ShapedFragWord), {})
    )
    V.append(v)
    K.append(klass)
    if len(K) >= 127:
...
ShapeData = namedtuple(
    "ShapeData", "cluster x_advance y_advance x_offset y_offset width"
)
...
class ShapedStr(str):
    ...
    def __add__(self, other):
        return ShapedStr(
            super().__add__(other),
            shapeData=(
                self.__shapeData__ + other.__shapeData__
                if isinstance(other, ShapedStr)
    ...
    def __radd__(self, other):
        return ShapedStr(
            str(other) + str(self),
            shapeData=(
                other.__shapeData__ + self.__shapeData__
                if isinstance(other, ShapedStr)
...
if not uharfbuzz:
    ...
    def shapeFragWord(w, features=dict(kern=True, liga=True, dlig=True), force=False):
        ...
        """
        if isinstance(w, ShapedFragWord):
            return w
        F = []
        text = ""
        specials = {}
        ...
        for i, (info, pos) in enumerate(zip(infos, positions)):
            ...
            shapeDataAppend(
                ShapeData(cluster, x_advance, y_advance, x_offset, y_offset, ucharWidth)
            )
        ...
        if specials:
            ...
            for _ in new:
                S.append(
                    ((_[1].__shapeData__[0].cluster, _[1].__shapeData__[-1].cluster), _)
                )
...
def shapeStr(s, fontName, fontSize, force=False):
    return shapeFragWord(
        [
            pdfmetrics.stringWidth(s, fontName, fontSize),
