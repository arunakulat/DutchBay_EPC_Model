...
class PlacedStory(Flowable):
    ...
    def drawOn(self, canv, lx, ly, _sW=0):
        ...
        if origin == "page":
            ...
            if not _:
                _ = self._doctemplateAttr("frame")
            dx = _.x1
            dy = _.y1
        else:
            raise ValueError(
                f'PlacedStory invalid {origin=} not in "page", "frame" or "local"'
            )
        ...
        if self.sb:
            canv.drawBoundary(self.sb, x, y, self.w, self.h)
...
class LIIndenter(DDIndenter):
    ...
    def split(self, aW, aH):
        ...
        if len(S) > 1 and hasattr(f, "blPara"):
            ...
            if (
                getattr(getattr(f, "style", None), "alignment", None) == 4
                and nnl >= (cnl - 2)
            ) or nnl == (cnl - 1):
                S = []
        return (
