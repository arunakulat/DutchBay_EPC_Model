...
from reportlab.pdfgen.textobject import (
    rtlSupport,
    bidiText,
    bidiWordList,
    isBidiStr,
...
def _splitFragWord(w, maxWidth, maxWidths, lineno):
    ...
    for g, cw, c in _fragWordIter(w):
        ...
        tooLong = newLineWidth > maxWidth and bool(
            wordWidth or (not wordWidth and cw <= maxWidthNext)
        )
...
def _hyphenateWord(
    ...
    if R:
        ...
        if isBidiStr(w):
            return [
                bidiStrWrap((_SplitWordHY if hy else _SplitWordH)(h + hy), w),
...
def _splitWord(w, lineWidth, maxWidths, lineno, fontName, fontSize, encoding="utf8"):
    ...
    for c in w:
        ...
        newLineWidth = lineWidth + cw
        if newLineWidth > maxWidth and (
            wordText or (not wordText and cw <= maxWidthNext)
        ):
    ...
    def breakLines(self, width):
        ...
        if nFrags == 1 and not (
                    ...
                    if doBidi:
                        ...
                        if shaping:
                            ...
                            words = [
                                innerBidiStrWrap(
                                    shapeStr(
                                        _, fontName=f.fontName, fontSize=v.fontSize
                                    ),
                                    VX[i],
                                    i,
                                )
            ...
            if bidiSort and not any((isBidiIndexList(_) for _ in _words)):
                ...
                if shaping:
                    _words = [
                        bidiIndexListWrap(shapeFragWord(_), BidiIndex(VX[i], i))
                        for i, _ in enumerate(_words)
                    ]
                else:
                    _words = [
                        bidiFragWord(_, direction=wordWrap, bidiV=VX[i], bidiL=i)
                        for i, _ in enumerate(_words)
                    ]
            elif shaping:
    ...
    def drawPara(self, debug=0):
        ...
        if nLines > 0:
            ...
            if blPara.kind == 0:
                ...
                tx.direction = (
                    wordWrap + "-handled" if wordWrap in ("LTR", "RTL") else wordWrap
                )
...
It should be a normal<onDraw name="_indexAdd" label="normal"/> paragraph, set in Courier 12 pt."""
    ...
    if flagged(12):
        ...
        P = Paragraph(
            text, style=ParagraphStyle("dvsstyle", fontName="DejaVuSans", fontSize=10)
        )
        dumpParagraphFrags(P)
        # w,h = P.wrap(6.27*72-12,10000)
        # dumpParagraphLines(P)
