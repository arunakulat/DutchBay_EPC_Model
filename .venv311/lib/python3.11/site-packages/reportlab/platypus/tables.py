...
ShadowStyle = namedtuple(
    "ShadowStyle",
    "dx dy color0 color1 nshades",
    defaults=(10, -10, "grey", "white", 30),
)
...
class Table(Flowable):
    def __init__(
        ...
        if shadow and not isinstance(shadow, ShadowStyle):
            raise ValueError(
                f"Table shadow argument should be None or a ShadowStyle instance not {shadow!r}"
            )
        self._shadow = shadow
    ...
    def _listCellGeom(self, V, w, s, W=None, H=None, aH=72000):
        ...
        if aW < 0:
            raise ValueError(
                f"{self.identity()}: flowable given negative availWidth={aW} == width={w} - leftPadding={s.leftPadding} - rightPadding={s.rightPadding}"
            )
        t = 0
        w = 0
        canv = getattr(self, "canv", None)
...
only rows may be strings with values in {_SPECIALROWS!r}"""
        ...
        if isinstance(value, (tuple, list)):
            ...
            for flowable, _fh in zip(value, FH):
                ...
                if split:
                    ...
                    usedHeight + flowable_height + flowable.getSpaceBefore() <= height0
                ):
                    ...
                    splits = flowable.split(
                        width, height0 - usedHeight - flowable.getSpaceBefore()
                    )
                    if splits:
    ...
    def _makeRoundedCornersClip(
        self, FUZZ=rl_config._FUZZ, clip=True, stroke=0, fill=0, deltas=(0, 0, 0, 0)
    ):
    ...
    def _drawCell(self, cellval, cellstyle, pos, size):
        ...
        if isinstance(cellval, (tuple, list, Flowable)):
            ...
            if direction:
                drawKwds["direction"] = direction
            shaping = cellstyle.shaping
            if shaping:
                drawKwds["shaping"] = shaping
            if drawKwds:
                for v in vals:
                    draw(x, y, v, **drawKwds)
                    y -= leading
