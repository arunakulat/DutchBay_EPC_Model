...
def _render_form() -> str:
    param_html = "\n".join(_input_row(k, '', SCHEMA[k]["unit"], f"[{SCHEMA[k]['min']}, {SCHEMA[k]['max']}]") for k in SCHEMA)
    debt_html = "\n".join(_input_row(f"debt.{k}", '', DEBT_SCHEMA[k]["unit"], f"[{DEBT_SCHEMA[k]['min']}, {DEBT_SCHEMA[k]['max']}]") for k in DEBT_SCHEMA)
...
def create_app() -> Any:
    ...
    async def run_baseline(payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        params = payload or {}
        return {k: v for k, v in build_financial_model(params).items() if k != "annual_data"}
    ...
    async def form_save(request: Request):
        ...
        if d: p["debt"] = d
        out_dir = Path("inputs/scenarios"); out_dir.mkdir(parents=True, exist_ok=True)
        (out_dir / outname).write_text(yaml.safe_dump(p, sort_keys=True), encoding="utf-8")
        return HTMLResponse(f"<p>Saved to inputs/scenarios/{outname}</p><p><a href='/form'>Back</a></p>")
