...
import numpy as np
import pandas as pd
from typing import Tuple, List, Dict, Any, Optional, Final
...
def calculate_irr_robust(
    ...
    if len(cf) < 2:
        return IRRResult(None, 'ERROR', method, None, 'Too few cash flows', 'Too few cash flows')
    if np.all(cf >= 0):
        return IRRResult(None, 'ERROR', method, None, 'No negative cash flows - IRR undefined', 'No negative cash flows')
    if np.all(cf <= 0):
        return IRRResult(None, 'ERROR', method, None, 'No positive cash flows - IRR undefined', 'No positive cash flows')
    ...
    if method in ['brentq', 'both']:
        try:
            irr_result = brentq(lambda r: calculate_npv(r, cf), -0.99, 5.00, xtol=tolerance, maxiter=max_iterations)
    ...
    if method in ['newton', 'both']:
        try:
            def npv_derivative(rate, cf):
                return -np.sum([i * c / (1 + rate) ** (i + 1) for i, c in enumerate(cf)])
            irr_result = newton(lambda r: calculate_npv(r, cf), initial_guess, fprime=lambda r: npv_derivative(r, cf), tol=tolerance, maxiter=max_iterations)
            npv_check = calculate_npv(irr_result, cf)
            if abs(npv_check) < tolerance * 10 and -0.99 < irr_result < 5.0:
                return IRRResult(irr_result, 'CONVERGED', 'Newton', npv_check, warning)
        except Exception as e:
            pass
    return IRRResult(None, 'FAILED', 'None', None, warning, 'All solver methods failed to converge')
...
def build_financial_model(
    proj: ProjectParameters = ProjectParameters(),
    ...
    # Operating metrics
    gen = np.array([proj.nameplate_mw * proj.hours_per_year * proj.cf_p50 * (1 - proj.yearly_degradation) ** (y - 1) for y in years])
    ...
    # OPEX escalation
    usd_portion = proj.opex_usd_mwh * proj.opex_split_usd * (1 + proj.opex_esc_usd) ** (years - 1)
    lkr_portion_base = proj.opex_usd_mwh * proj.opex_split_lkr * (1 + proj.opex_esc_lkr) ** (years - 1)
    ...
    for year in range(1, proj.project_life_years + 1):
        usd_int = usd_bal * USD_MKT_RATE  # For simplicity, use market USD rate as default
