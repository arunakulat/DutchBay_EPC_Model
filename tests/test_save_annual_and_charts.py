...
def test_scenarios_save_annual(tmp_path: Path):
    ...
    cmd = [sys.executable, "-m", "dutchbay_v13", "scenarios", "--format", "csv", "--save-annual", "--outdir", str(outdir)]
...
def test_charts_generation(tmp_path: Path):
    ...
    cmd = [sys.executable, "-m", "dutchbay_v13", "baseline", "--outdir", str(outdir), "--charts"]
    r = subprocess.run(cmd, capture_output=True, text=True)
    assert r.returncode == 0
